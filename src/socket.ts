...
class SocketController {
  ...
  initEventListeners() {
    this.io.on("connection", (socket) => {
      socket.on(
        "send_message",
        async (
          { text, image }: MessageData,
          cb?: (arg: Record<string, any>) => void,
        ) => {
          try {
            ...
            if ((err as any).msBeforeNext) {
              const secondsBeforeNext = Math.ceil(
                (err as any).msBeforeNext / 1000,
              );
              socket.emit("message", {
                text:
                  `Ditt meddelande blev blockerat då du skickar för många i snabb följd. Skriv hellre längre meddelanden än att skicka många korta. Försök igen om ${secondsBeforeNext} sekund${
                    secondsBeforeNext > 1 ? "er" : ""
                  }.`,
                name: "DoTheMath",
              });
            }
      ...
      socket.on(
        "establish_session",
        ({ studentName, channelId }: EstablishSessionData, cb?: () => void) => {
      ...
      socket.on(
        "reestablish_session",
        async (
          { threadId, channelId }: ReEstablishSessionData,
          cb?: (arg: Record<string, any>) => void,
        ) => {
          try {
  ...
  async sendMessage(
    { sender, senderAvatar, text, socketId, image }: SendMessageOptions,
  ) {
