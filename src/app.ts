...
const SLACK_CHANNELS_FIX_AZURE_ENCODING =
  process.env.SLACK_CHANNELS_FIX_AZURE_ENCODING === "true";
const SLACK_CHANNELS_FIXED = SLACK_CHANNELS_FIX_AZURE_ENCODING
  ? fixAzureAppSettingsEncoding(SLACK_CHANNELS_RAW)
  : SLACK_CHANNELS_RAW;
...
class AppController {
  ...
  async reEstablisSession(
    { socketId, channelId, threadId }: ReEstablishSessionOptions,
  ) {
    _.remove(
      this.sessions,
      (session) =>
        session.socketId === socketId ||
        (session.channelId === channelId && session.threadId === threadId),
    );
  ...
  dropSession(socketId: string) {
    const droppedSessions = _.remove(
      this.sessions,
  ...
  async handleMessageFromClient(
    { text, image, socketId }: HandleMessageFromClientOptions,
  ) {
    console.log("handleMessageFromClient");
   ...
   */
  async handleMessageFromSlack(
    { text, threadId, sender, senderAvatar, image }:
      HandleMessageFromSlackOptions,
  ) {
